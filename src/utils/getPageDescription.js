const getPageDescription = (page) => {
  const sections = page.sections.filter(section => {
    return section.items.filter(({ item }) => {
      return item.values.pText && item.values.pText.length > 9
    })
  })
  const items = sections.filter(section => section.items.find(({item}) => item.values.pText && item.values.pText.length > 9))
  if (items[0]) {
